...
//        int nn = list.size();
//
//        Log.w(TAG, "list.size --> "+ nn);
//        String ClientCharacteristicConfig = list.get(0).getUuid().toString();
//        Log.w(TAG, "list.size aaa--> "+ aaa);
//        mBluetoothGatt.setCharacteristicNotification(characteristic, enabled);

//        BluetoothGattCharacteristic characteristic = gatt.getService(serviceUuid).getCharacteristic(characteristicUuid);

        if (UUID.fromString("468e6033-aa75-2215-88ca-f9cfbb2575d5").equals(characteristic.getUuid())) {
            mBluetoothGatt.setCharacteristicNotification(characteristic, enabled);
...
//            Log.w(TAG, "list.size --> "+ nn);
            if(list.size() > 0) {
                UUID ClientCharacteristicConfig = list.get(0).getUuid();
                Log.w(TAG, "ClientCharacteristicConfig --> " + ClientCharacteristicConfig.toString());
...
//            BluetoothGattDescriptor descriptor = characteristic.getDescriptor(UUID.fromString(ClientCharacteristicConfig));
                ...
                for (byte b : BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE) {
                    Log.d("ENABLE_NOTIFICATION", String.format("0x%02x", b));
                }
                for (byte b : BluetoothGattDescriptor.DISABLE_NOTIFICATION_VALUE) {
                    Log.d("DISABLE_NOTIFICATION", String.format("0x%02x", b));
                }

                BluetoothGattDescriptor descriptor = characteristic.getDescriptor(ClientCharacteristicConfig);
                descriptor.setValue(enabled ? BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE : BluetoothGattDescriptor.DISABLE_NOTIFICATION_VALUE);
//            descriptor.setValue(new byte[]{0x00, 0x01});
